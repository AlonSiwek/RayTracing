...
public class Plain extends Shape {
	...
	public Vec normal(Ray ray) {
		return ray.direction().dot(normal()) < 0.0D ? normal() : normal().neg();
	}
	...
	public Hit intersect(Ray ray) {
		...
		double sub = (abc.dot(ray.source().toVec()) + d) * (-1);
		double direction =  sub / ray.direction().dot(abc);

		return ((direction > 1.0E-5D ? 1 : 0) & (direction < 1.0E8D ? 1 : 0)) != 0 ? new edu.cg.algebra.Hit(direction, normal(ray)) : null;
	}
}
